...
impl ShuffleSolver {
    pub fn solve(options: ShuffleSolverOptions) -> ShuffleSolution {
        ...
        ShuffleSolution {
            required_moves: solver.required_moves,
        }
    }
    ...
    fn solve_internal(&mut self) {
        ...
        for next_desired_output_index in (0..self.desired_draw_order.len()).rev() {
            ...
            let which_stack_index = self
                .find_stack_index_with_card(next_desired_output)
                .unwrap();
    ...
    fn validate_solution(&self) {
        let mut other = ShuffleSolver::new_internal(ShuffleSolverOptions { ..self.options });
    ...
    fn move_card(&mut self, src_stack_index: usize, dst_stack_index: usize) {
        ...
        self.required_moves.push(CardMove {
            src_stack: src_stack_index,
            dst_stack: dst_stack_index,
        });
    }
}
